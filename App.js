/**
 * Sample React Native App
 * https://github.com/facebook/react-native
 *
 * @format
 * @flow strict-local
 */

 import React from 'react';

 import {
   SafeAreaView,
   ScrollView,
   StatusBar,
   StyleSheet,
   Text,
   useColorScheme,
   View,
   Image,
   TextInput,
   Pressable,
   Alert,
 } from 'react-native';
 
 const App = () => {
   const login = ()=>{
     Alert.alert("Confirmation", "Successfully login")
   }
   
 
   return (
     <SafeAreaView style={{flex:1,backgroundColor:"black"}}>
       <View>
         <View>
           <View style={{marginTop:30,padding:10,justifyContent:"center",alignItems:"center"}}><Text style={{color:"#02d9b1",fontSize:32,fontFamily:"system-ui",fontWeight:"bold", }}>LOGIN</Text></View>
           <View style={{marginTop:30,marginLeft:20,padding:10,justifyContent:"center"}}><Text style={{color:"grey",fontSize:16 }}>Login with one of the following account</Text></View>
           <View style={{marginTop:10,marginLeft:20,padding:10,justifyContent:"space-evenly",flexDirection:"row",alignItems:"center"}}>
             <View style={{backgroundColor:"#4c4d4c",borderRadius:20,width:150,height:60,justifyContent:"center",alignItems:"center",elevation:30}}>
               <Image  style={styles.googleLogo} source={{uri:"https://icon-library.com/images/google-g-icon/google-g-icon-9.jpg"}} />
               </View>
            <View  style={{backgroundColor:"#4c4d4c",borderRadius:20,width:150,justifyContent:"center",alignItems:"center",elevation:30}}>
              <Image style={styles.appleLogo} source={{uri:"data:image/png;base64,"}} />
            </View>
           </View>
             <View style={{marginTop:30,marginLeft:15,}}><Text style={{fontSize:20, fontWeight:"bold",color:"white" }}>  Email</Text></View>
           <View style={{marginTop:10,marginLeft:20,marginRight:10,padding:10,justifyContent:"center",borderColor:"#02d9b1",borderWidth:2,borderRadius:10,}}>
             <TextInput placeholder='someone@example.com' placeholderTextColor="white"></TextInput>
             </View>
             <View style={{marginTop:15,marginLeft:15}}><Text style={{fontSize:20, fontWeight:"bold",color:"white" }}>  Password</Text></View>
           <View style={{marginTop:10,marginLeft:20,marginRight:10,padding:10,justifyContent:"center",borderColor:"#02d9b1",borderWidth:2,borderRadius:10,elevation:30}}>
             <TextInput placeholder='********' placeholderTextColor="white"></TextInput>
             </View>
 
             <View style={{marginTop:30,marginLeft:20,marginRight:10,padding:10,justifyContent:"center",backgroundColor:"#02d9b1",alignItems:"center",borderRadius:20,elevation:30}}>
               <Pressable onPress={login}><Text style={{fontSize:20, fontWeight:"bold",color:"white"}}>Login</Text></Pressable>
             </View>
 
             <View style={{marginTop:20,marginLeft:15,justifyContent:"center",alignItems:"center",flexDirection:"row"}}><Text style={{fontSize:16,color:"white" }}>Don't have an account ?</Text>
             <Text style={{fontSize:16,color:"#02d9b1",marginLeft:2 }}>Create new account</Text></View>
 
         </View>
       </View>
     </SafeAreaView>
   );
 };
 
 
 const styles = StyleSheet.create({
     googleLogo:{
       width:40,
       height:40,
     },
     appleLogo:{
       width:60,
       height:60,
     },
 })
 
 
 export default App;
 